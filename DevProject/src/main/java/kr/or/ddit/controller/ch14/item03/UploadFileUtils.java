...
public class UploadFileUtils {
	public static String uploadFile(String uploadPath, String originalName, byte[] fileData) throws Exception {
		...
		FileCopyUtils.copy(fileData, target); //위에서 만들어진 경로를 가지고 파일 복사 진행
		String formatName = originalName.substring(originalName.lastIndexOf(".") + 1);
		
		String uploadedFileName = savedPath.replace(File.separatorChar, '/') + "/" + savedName;
		
		//확장자가 이미지 파일이면 s_가 붙은 파일의 썸네일 이미지 파일을 생성
	...
	private static void makeThumbnail(String uploadPath, String path, String fileName) throws Exception {
		...
		//targetSize: 100 : 정사각형 사이즈로 100x100
	...
	/**
	 * : 2023/06/07 폴더 경로를 만들고
	 * /2023/06/07 폴더 경로를 리턴하는 메소드
	 * 
	 * @param uplaodPath 저장 경로
	 * @return 저장경로/년/월/일
	 */
	private static String calcPath(String uploadPath) {
		...
		//DemicalFormat("00") : 두자리에서 빈자리는 0으로 채움
		//ex) 01,02,03
	...
	//키워드 '...'를 사용
	//[사용법] 타입 ...변수명 형태
	//순서대로 yearPath, monthPath, datePath 가 배열로 들어가 처리
