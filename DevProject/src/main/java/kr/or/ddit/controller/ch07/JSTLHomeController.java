...
public class JSTLHomeController {
	 ...
	 * 이런 중복되는 노력을 없애기 위해서 나온것이 바로 JSTL이다
	 * 
	 * 	1) core 태그 라이브러리
	 *   - <c:out> : JSP Writer에 값을 적절하게 처리한 후에 출력하낟
	 *   - <c:set> : JSP에서 사용할 변수를 설정한다(setter)
	 *   - <c:remove> : 설정한 변수를 제거한다
	 *   - <c:catch> : 예외를 처리한다
	 *   - <c:if> : 조건을 지정하고 지정된 조건과 일치하는 처리 내용을 구현한다
	 *   - <c:choose> : 여러 조건을 처리할 때 사용한다
	 *   - <c:when> : 여러 조건을 지정하고 지정한 조건과 일치하는 처리 내용을 구현한다. <c:choose>안에서 사용된다
	 *   - <c:otherwise> : <c:when>요소에서 지정한 조건에 모두 일치하지 않을 때 처리할 내용을 구현한다. <c:choose>안에서 사용된다
	 *   - <c:forEach> : 컬렉션이나 배열의 각 항목을 처리할 떄 사용한다
	 ...
	 *   
	 *  6. 코어 태그
	 *   - 조건 분기나 반복처리 그리고 변수의 지정 등과 같이 논리적인 처리를 위해 사용되는 스크립트 코드를 대체하기 위한 태그를 제공
	 *   	1) <c:out>
	 *   	- JSPWriter에 값을 적적ㄹ하게 처리한 후에 출력
	 *   		- value(Object) : 출력할 값
	 ...
	 *   	3) <c:remove>
	 *   	- 설정한 변수를 제거한다
	 *   		- var(String) : 삭제할 EL 변수명
	 *   		- scope(String) : 삭제할 변수가 포함된 영역
	 *   
	 *    	4) <c:catch>
	 *    	- 예외를 처리한다
	 *    		- var(String) : 예외를 지정할 EL 변수 명
	 *    	
	 *    	5) <c:if>
	 *    	- 조건을 지정하고 지정된 조건과 일치하는 처리 내용을 구현
	 *    		- test(boolean) : 검사 조건
	 ...
	 *    
	 *    	7) <c:when>
	 *    	- 여러 조건을 지정하고 지정한 조건과 일치하는 처리 내용을 구현
	 *    		- test(boolean) : 검사조건
	 *    
	 *    	8) <c:otherwise>
	 *    	- <c:when> 요소에 지정한 조건에 모두 일치하지 않을 때 처리 내용을 구현
	 *    	
	 *    	9) <c:forEach>
	 *    	- 컬렉션이나 배열의 각 항목을 처리할 때 사용
