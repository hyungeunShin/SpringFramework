...
public class TransactionController {
	 ...
	 * 	- 격리성(Isolation)
	 * 		> 트랜잭션이 처리되는 동안 외부에서의 간섭이 없어야 한다
	 * 	- 영속성(Durability)
	 ...
	 * 		[ch12] 참조
	 * 		
	 * 		ex) 회원은 반드시 하나의 권한을 가진다는 규칙이 있다.
	 * 		회원과 회원권한 테이블이 각각 개별로 존재하지만 회원정보를 저장할 때는 반드시 회원권한 정보를 동시에 저장해야 한다
	 ...
	 * 		회원 정보와 권한 정보 모두 저장에 성공해야 비로소 회원 테이블과 회원 권한 테이블에 저장이 이루어진다
	 * 		삭제, 수정도 마찬가지
	 * 
	 * 	@Transactional 을 적용하고 중간에 에러를 발생 시킨 후, 롤백된 상태를 확인하려고 했으나 롤백이 되지 않고 db에 저장 됨
	 * 
	 ...
	 * 
	 *  여기서 트랜잭션으로 국한된 롤백 정책이 아니라 스프링 프레임워크에서의 기본 정책에 대한 내용
	 *  실제로 '트랜잭션 롤백 처리', 'Checked Exception vs UnChecked Exception' 등 검색해보면 잘못된 정보들이 많다(특히, 표)
	 *  
	 *  기본적인 트랜잭션의 정책이 있고 그 안에서 정책 이외의 옵션으로 특정 에러가 발생했을때 롤백을 진행할 수 있도록 제공한다.
	 *  
	 *  Exception(예외)와 Error(에러)
	 *  - Exception : 개발 알고리즘에서 발생하는 오류로 개발자가 작성한 코드에서 발생하므로 예외를 상황에 맞춰 처리할 수 있다
	 *  - Error : 시스템에서 발생하는 심각한 수준의 에러로 개발자가 미리 예측하여 대응 할 수 없기 때문에 예외 처리에 대한 부분을 신경쓰지 않아도 된다
	 *  
	 *  Checked Exception vs UnChecked Exception
	 *  - RuntimeException의 상속 여부에 따라서 Checked Exception과 Unchecked Exception으로 나누어진다
	 *  
	 *  [예외 처리 여부]
	 *  Checked Exception : 반드시 예외 처리 코드가 있어야 한다
	 *  UnChecked Exception : 명시적인 처리를 강제하지 않는다
	 *  
	 *  [예외 확인 시점]
	 *  Checked Exception : 컴파일 단계에서부터 컴파일이 되지 않는다
	 *  UnChecked Exception : 런타임중 예외가 확인된다
	 *  
	 *  [클래스]
	 *  Checked Exception : IOException, SQLException...
	 *  UnChecked Exception : NullPointException, IndexOutOfBoundException..
	 *  
	 *  트랜잭션 또한 AOP의 개념이 반영된 관점 지향 프로그래밍이라 할 수 있다
	 *  
	 *  선언적 트랜잭션(@Transactional)
	 *  - 컨트롤러 메소드 각 단위로 세밀한 트랜잭션 속성 제어가 가능
	 *  - 해당 어노테이션이 클래스 수준에서 선언되면 선언 클래스 및 해당 하위 클래스의 모든 메소드에 기본값으로 적용
	 *  - RuntimeException계열, Error예외에 대해서는 Rollback가능
	 *  
	 *  	1) isolation(격리 수준)
	 *  		- 각 트랜잭션이 존재할 때, 트랜잭션들 끼리 서로 고립된 수준을 나타내며 서로간에 가용된 데이터를 컨트롤 할지에 대한 부분들을 설정할 수 있다
	 *  		- isolation 기본 값은 default이다
	 *  		- 새롭게 시작된 트랜잭션에만 적용되므로, Propagation.Required 또는 Propagation.REQUIRES_NEW 와 함께 사용되도록 독점 설계되었다
	 *  		
	 *  		(용어)
	 *  		# Dirty read
	 ...
	 *  		- 처리중인 트랜잭션이 다른 트랜잭션이 Commit한 데이터를 읽을 수 있는 것을 의미
	 *  		
	 *  		#Phantom Read
	 ...
	 *  		- READ_COMMITED : Dirty read가 방지됨을 나타내는 상수
	 *  						  Non-repeatable read 및 Phantion Read가 발생할 수 있다
	 *  			> 하나의 트랜잭션 처리가 이루어진 변경 내용이 Commit 된 후, 다른 트랜잭션에서 조회가 가능
	 ...
	 *  
	 *  	2) propagation(전파옵션)
	 *  	- 기존 진행중인 트랜잭션 외에 추가적으로 진행중인 트랜잭션이 존재할 떄 추가적인 트랜잭션에 대해서 어떻게 처리할지에 대한 설정
	 *  	- 추가적인 트랜잭션을 기존 트랜잭션에 포함 시켜 함께 처리할 수도 있고, 추가적인 트랜잭션처럼 별도의 트랜잭션으로 추가 할 수도 있고
	 *  	다른 트랜잭션처럼 진행되다 에러를 발생 시킬 수 있다
	 *  
	 *  	[옵션]
	 *  	-REQUIRED : 현재 트랜잭션을 지원하고 존재하지 않는 경우 새 트랜잭션을 만든다
	 *  		> propagation 기본 default 옵션에 해당
	 *  		> 부모/자식 간에 상관관계에서 자식부분의 트랜잭션이 rollback 처리 시, 부모도 rollback 처리된다
	 ...
	 *  	-NESTED : 현재 트랜잭션이 있는 겨웅 중첩된(부모-자식) 트랜잭션 내에서 실행하고 그렇지 않은 경우 REQUIRED와 같이 동작한다
	 *  		> 부모에서 예외가 발생했을때, 자식까지 영향이 가서 Commit되지 않는다 
	 *  	-NEVER : 트랜잭션이 아닌 방식으로 실행하고 트랜잭션이 있으면 예외를 발생시킨다
	 *  		> 실행 자체가 트랜잭션을 필요로하지 않고 트랜잭션이 존재한다면 예외를 발생시킨다
	 ...
	 *  	- 기본값은 false
	 *  	- readOnly 속성을 설정했다고 해서, 읽기 전용으로 무조건 설정된다는 보장이 없음(쓰기와 같은 트랜잭션이 실행될 수도 있다)
	 *  		> 읽기 전용에 대한 힌트를 분석할 수 없는 트랜잭션인 경우 throw 를 던지지 않고 조용히 힌트를 무시
	 *  
	 *  	4) timeout(트랜잭션 제한시간)
	 *  	- 기본값은 -1로 무제한
	 ...
	 *  	- 기본적으로 트랜잭션은 롤백 되지만, RuntimeException 계열과 Error은 롤백되지 않는다
